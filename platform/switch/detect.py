...
def can_build():
    ...
    if "DEVKITPRO" not in os.environ:
        print("DEVKITPRO not defined in environment...")
        _can_build = False
...
def get_opts():
    ...
    return [
        ...
        BoolVariable("audren", "Use the Horizon audio driver", True),
    ]
...
def configure(env):
    ...
    os.environ["DKP_TOOLS_BIN"] = "{}/tools/bin".format(dkp)
    ...
    if env["audren"]:
        env.Append(CPPDEFINES=["AUDREN_ENABLED"])
